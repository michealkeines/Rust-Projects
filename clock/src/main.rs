...
impl Clock  {
    ...
    fn set<Tz: TimeZone>(t: DateTime<Tz>) -> () {
        ...
        let is_leap_season = ns > 1_000_000_000;
...
fn main() {
    let app = App::new("clock")
        .version("0.1.3")
        .about("Get and Sets the time.")
        .after_help("Note: UNIX timestamps are parsed as whole seconds since 1 january 1970 0:00:00 UTC. For more accuracy, use another formatt.")
        .arg(Arg::with_name("action").takes_value(true).possible_values(&["get", "set", "check-ntp"]).default_value("get"))
        .arg(Arg::with_name("std").short("s").long("use-standard").takes_value(true).possible_values(
            &["rfc2822", "rfc3339", "timestamp"]
