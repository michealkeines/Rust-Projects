...
fn parse(input: &str) -> Vec<Operation> {
    ...
    for (i, byte) in input.bytes().enumerate() {
        let current = Work::Task(i, byte);
        todo_tx.send(current).unwrap();
        n_bytes += 1;
    }
    ...
    for _ in 0..n_bytes {
        let (i, op): (usize, Operation) = results_rx.recv().unwrap();
        ops[i] = op;
    }
    ops
...
fn main() {
    ...
    let default_file = format!("{}.svg", input);
    let save_to = args.get(2).unwrap_or(&default_file);
